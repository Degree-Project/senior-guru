...
exports.getSingleReservationDetails = asyncErrorHandler(async (req, res, next) => {

    const reservation = await Reservation.findById(req.params.id).populate("user", "firstName lastName email avatar");
...
exports.myGuruReservations = asyncErrorHandler(async (req, res, next) => {
    ...
    const reservations = await Reservation.find({ 'reservationItem.guru': req.user._id });
